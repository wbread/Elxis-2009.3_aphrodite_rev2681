...
class Snoopy 
{
	...
	public $agent			= "Mozilla/5.0 (Windows; U; Windows NT 6.1; en; rv:1.8) Gecko/20061107 Firefox/3.0";
	...
	public $maxredirs		=	1;					// http redirection depth maximum. 0 = disallow
	...
	// http accept types
	public $accept			=	"image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, image/png, */*";
					...
					if($this->_redirectaddr)
					{
						...
						{						
							...
							if(preg_match("|^http://".preg_quote($this->host)."|i",$this->_redirectaddr) || $this->offsiteok)
							{
								...
								$this->_redirectdepth++;
								$this->lastredirectaddr=$this->_redirectaddr;
								if( $this->utf8_strpos( $this->_redirectaddr, "?" ) > 0 )
	...
	function _httprequest($url,$fp,$URI,$http_method,$content_type="",$body="")
	{
		...
		{			
			...
			if ( count($this->cookies) > 0 ) {
				...
				$headers .= $this->utf8_substr($cookie_headers,0,-2) . "\r\n";
			} 
		}
		...
		if(!empty($body))	
			$headers .= "Content-length: ".$this->utf8_strlen($body)."\r\n";
		...
		fwrite($fp,$headers.$body,$this->utf8_strlen($headers.$body));
	...
	function _httpsrequest($url,$URI,$http_method,$content_type="",$body="")
	{
		...
		{			
			...
			if ( count($this->cookies) > 0 ) {
				...
				$headers[] = $this->utf8_substr($cookie_str,0,-2);
			}
		}
		if(!empty($this->rawheaders))
		{
...
*/
	function utf8_strpos($haystack, $needle,$offset=0) {
	    ...
	    if(!$offset){
	        $ar = $this->utf8_explode($needle, $str);
	        if ( count($ar) > 1 ) {
	        ...
	        if ( false !== ($pos = $this->strpos($str,$needle))){
	            return $pos + $offset;
	        }
	        return false;
	    }
