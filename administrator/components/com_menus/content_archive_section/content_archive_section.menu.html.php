...
* @description: Creates a menu item that links to an archive section
*/
	...
	static public function editSection( &$menu, &$lists, &$params, $option ) {
			...
			if ( !$menu->id ) {
				...
				if ( form.name.value == '' ) {
					if ( form.componentid.value == 0 ) {
						form.name.value = '<?php echo $adminLanguage->A_ALL_SECTIONS2; ?>'
					} else {
		...
		<tr>
			<th>
                ...
                <?php echo $adminLanguage->A_CMP_MU_MITEM.' : '.$adminLanguage->A_BLOG.' - '. $adminLanguage->A_MENUS_CNTSCARC; ?>
			</th>
		</tr>
		</table>
