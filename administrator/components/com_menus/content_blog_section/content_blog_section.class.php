...
class content_blog_section {
	...
	static public function edit( $uid, $menutype, $option ) {
		...
		if ($uid) {
			...
			if ( $secids ) {
				...
				. "\n FROM #__sections s"
				. "\n WHERE s.scope = 'content' AND s.id IN ( ". $secids . ")"
				. "\n ORDER BY s.name";
		...
		$rows[] = mosHTML::makeOption( '', $adminLanguage->A_ALL_SECTIONS );
		$query = "SELECT s.id AS value, s.id AS id, s.title AS text FROM #__sections s"
		."\n WHERE s.scope = 'content' ORDER BY s.name";
		$database->setQuery( $query );
		$rows = array_merge( $rows, $database->loadObjectList() );
		$section = mosHTML::selectList( $rows, 'secid[]', 'class="selectbox" size="10" multiple="multiple"', 'value', 'text', $lookup );
	...
	static public function saveMenu( $option, $task ) {
		...
		if ( count( $secids )== 1 && $secids[0] != '' ) {
			$row->link = eUTF::utf8_str_replace( 'id=0','id='. $secids[0], $row->link );
