...
/* 
@ Author: Ioannis Sannos (blackgate@datahellas.com) - Elxis team
@ Date: 29.06.2006
@ Note: Class updated and enriched for use in Elxis CMS
*/
...
class FloodBlocker {
    ...
    public function __construct() {
        ...
        $configpath = $elxis_root.'/administrator/tools/floodblocker/config.php';
        ...
        if ( !$this->init_error ) {
            include ( $configpath );
            $this->logs_path = $elxis_root.'/administrator/tools/floodblocker/logs/';
            if (!is_dir ($this->logs_path)) {
                $this->init_error = 1;
            }
            
            $this->floodblock_enabled = intval($fconf_enabled);
            $this->cron_file = '.time';
            $this->cron_interval = intval($fconf_interval);
            $this->logs_timeout = intval($fconf_timeout);
            $this->rules = $fconf_rules;

            if ((!is_array($this->rules)) || (count($this->rules) == '0')) {
                $this->init_error = 1;            
            }
        }
        if ((!$this->init_error) && ($this->floodblock_enabled)) {
            $this->FloodBlockerRun();
        }
    }


    function FloodBlockerRun ( $ip = '' ) {
        if ( empty ( $ip ) ) {
            $ip = $_SERVER['REMOTE_ADDR'];
        }
        $ip = ip2long ( $ip );
        if ( $ip == -1 || $ip === FALSE ) {
            trigger_error ( FLOOD_IP_ADDR, E_USER_ERROR );
        }
        $this->ip_addr = $ip;
    }
    ...
    /*
    Description:
    Used to check flooding. Generally this function acts as private method and will be called 
    internally by public methods. However, it can be called directly when storing logs in db.
    Prototype:
    ...
    */
    function CheckCron() {
        ...
        if ( substr ( $this->cron_file, 0, 1 ) != '.' ) {
            trigger_error ( FLOOD_CRON_FNAME, E_USER_WARNING );
            return;
        }
        ...
        if ( ! ( $f = @fopen ( $path, 'a+' ) ) ) {
            trigger_error ( FLOOD_CRON_FILE, E_USER_WARNING );
            return;
        }
    ...
    */
    function CronJob() {
        ...
        if ( ! ( $dir_hndl = @opendir ( $this->logs_path ) ) ) {
            trigger_error ( FLOOD_CRON_JOB, E_USER_WARNING);
            return;
        }

        while ( $fname = @readdir ( $dir_hndl ) ){
            if (( substr( $fname, 0, 1 ) == '.' ) || ($fname=='index.html')) {         
                continue;
            }
            clearstatcache ( );
