...
class Container {
	...
	function emptyParagraph(&$font, &$parFormat) {
		if (($parFormat instanceof ParFormat) && ($font instanceof Font)) {		  	
		  	//$content = (count($this->elements) != 0 && empty($this->emptyPar)) ? '\par ' : '';	
        $content = (($parFormat instanceof ParFormat) && count($this->elements) != 0 && empty($this->emptyPar)) ? '\par ' : '';			
	...
	function writeText($text, &$font, &$parFormat, $replaceTags = true) {				
		...
		$content = (($parFormat instanceof ParFormat) && count($this->elements) != 0 && empty($this->emptyPar)) ? '\par ' : '';	
		$this->emptyPar = false;  	
		$content .= ($parFormat instanceof ParFormat) ? $this->pard.$parFormat->getContent($this->rtf) : '';	 					
		$content .=	'{';
		...
		foreach($this->elements as $key => $value) {			
			if (is_string($value)) {	
				...
				if ($key != 0 
					...
					&& !($this->elements[$key - 1] instanceof Table)) 
				{				  	
				  	$content .= '\par';
				} else if (($value instanceof Image)) {	
					if (($value->parFormat instanceof ParFormat)) {
					 	$content .= $key != 0 ? '\par' : '';
