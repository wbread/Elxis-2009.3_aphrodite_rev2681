...
/**
Changes made by Ioannis Sannos:
PHP 4 compatibility and better table handling
*/ 
...
if(!class_exists('TCPDF', false)) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 3.0.007 (http://www.tcpdf.org)');
	...
	*/
    ...
    /**
        ...
        /**
		* Internal proccessing table number (for html input)
		* First table has value 0
		* Added by Ioannis Sannos
		*/
        protected $tbl_internal = 0;

        /**
		* Index of number of columns for each proccessing table (for html input)
		* Added by Ioannis Sannos
		*/
        protected $td_index = array();		
		...
		*/
        ...
        /**
		* Tables Proccessor
		* Takes counted tables and columns in an array and sets accordingly 
		* default_table_columns parameter for WriteHTML() function
		* Author: Ioannis Sannos (blackgate@datahellas.com)
		*/
        function proccessTables( $input=array() ) {
            $this->td_index = $input;
            $this->tbl_internal = 0;
        }		
			...
			switch($dest) {
				case 'I': {
					...
					if(php_sapi_name()!='cli') {
						...
						/**
						* Addittion by Ioannis Sannos
						* If user agent is I.Explorer pass cache-control in headers
						*/
                        if (phpversion() <= "4.2.1") {
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		protected function openHTMLTagHandler($tag, $attr, $fill=0) {
			...
			switch($tag) {
				...
				case 'font': {
					if (isset($attr['face'])) {
						$fontslist = preg_split("/\,/", strtolower($attr['face']));
