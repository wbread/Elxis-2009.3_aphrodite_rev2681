...
class adoSchema {
	...
	public $sqlArray; //private
	public $db; //private
	public $currentElement = ''; //private
	public $mgq; //private
	public $debug; //private
	public $versionRegex = '/<schema.*?( version="([^"]*)")?.*?>/'; //private
	public $schemaVersion; //private
	public $upgrade = '';
	public $dict;	
	public $objectPrefix = '';	
	public $success;
	...
	/********************************/
	/* adoSchema OBJECT CONSTRUCTOR */
	/********************************/
	public function __construct( &$db ) {
		//Initialize the environment
		$this->mgq = get_magic_quotes_runtime();
		//set_magic_quotes_runtime(0); //deprecated in php5.3
		$this->db = $db;
	...
	function ExtractSchema( $data = FALSE, $indent = '  ', $prefix = '' , $stripprefix=false) {
		...
		if( is_array( $tables = $this->db->MetaTables( 'TABLES' , ($prefix) ? $prefix.'%' : '') ) ) {
			foreach( $tables as $table ) {
				...
				if (in_array($this->db->databaseType, array('oci8', 'oci805', 'oci8po', 'oracle'))) {
	...
	*/
	function Destroy() {
		//set_magic_quotes_runtime( $this->mgq ); //deprecated in php5.3
		unset( $this );
	}
}
