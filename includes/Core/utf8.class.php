...
* @description: eUTF class for handling utf-8 encoded strings
* most functions by Andreas Gohr <andi@splitbrain.org>
 ...
 * @see urlencode
 */
static function utf8_encodeFN($file,$safe=true){
 ...
 * @see    urldecode
 */
static function utf8_decodeFN($file){
  $file = urldecode($file);
  return $file;
}
 ...
 */
static function utf8_isASCII($str){
 ...
 */
static function utf8_strip($str){
  $ascii = '';
  for($i=0; $i<strlen($str); $i++){
    if(ord($str{$i}) <128){
      $ascii .= $str{$i};
 ...
 */
static function utf8_substr_replace($string, $replacement, $start , $length=0 ){
  $ret = '';
  if($start>0) $ret .= eUTF::utf8_substr($string, 0, $start);
  $ret .= $replacement;
  $ret .= eUTF::utf8_substr($string, $start+$length);
  return $ret;
}

/**
 * Unicode aware replacement for explode()
 * explode and utf-8: As all arguments used are well formed UTF-8, no problems.
 */
static function utf8_explode($sep, $str) {
  if ( $sep == '' ) {
 ...
 * str_replace and utf-8: As all arguments used are well formed UTF-8, no problems.
 */
static function utf8_str_replace($s,$r,$str){
  if(!is_array($s)){
    $s = '!'.preg_quote($s,'!').'!u';
  }else{
    foreach ($s as $k => $v) {
...
static function  utf8_trim($str,$charlist='') {
  ...
  return eUTF::utf8_ltrim(eUTF::utf8_rtrim($str));
}

/**
 * This is a unicode aware replacement for strtolower()
 * Uses mb_string extension if available
 * @see strtolower(), utf8_strtoupper()
 */
static function utf8_strtolower($string){
  ...
  $uni = eUTF::utf8_to_unicode($string);
  $cnt = count($uni);
  for ($i=0; $i < $cnt; $i++){
  ...
  return eUTF::unicode_to_utf8($uni);
}

/**
 * This is a unicode aware replacement for strtoupper()
 * Uses mb_string extension if available
 * @see strtoupper(), utf8_strtoupper()
 */
static function utf8_strtoupper($string){
  if(!defined('UTF8_NOMBSTRING') && function_exists('mb_strtolower'))
 ...
 */
static function utf8_stripspecials($string,$repl='',$additional=''){
  global $UTF8_SPECIAL_CHARS;

  static $specials = null;
  if(is_null($specials)){
    $specials = preg_quote(eUTF::unicode_to_utf8($UTF8_SPECIAL_CHARS), '/');
  }
 ...
 * Uses mb_string extension if available
 * @see strpos()
 * strpos behave correctly with well formed UTF-8 but the result it returns is in bytes not characters
 */
static function utf8_strpos($haystack, $needle,$offset=0) {
    if(!defined('UTF8_NOMBSTRING') && function_exists('mb_strpos')) {
        return mb_strpos($haystack,$needle,$offset,'UTF-8');
    }

    if(!$offset){
        $ar = eUTF::utf8_explode($needle, $haystack);
        if ( count($ar) > 1 ) {
           return eUTF::utf8_strlen($ar[0]);
        }
        return false;
    } else {
        if ( !is_int($offset) ) {
        ...
        $str = eUTF::utf8_substr($haystack, $offset);
        if ( false !== ($pos = eUTF::utf8_strpos($str,$needle))){
            return $pos + $offset;
        }
        return false;
    }
 ...
 * Uses mb_string extension if available
 * @see strrpos()
 * strrpos() will return an answer in bytes not characters.
 */
static function utf8_strrpos($haystack, $needle, $offset=0) {
 ...
 */
static function utf8_tohtml ($str) {
  $ret = '';
  $max = strlen($str);
 ...
 * @see utf8_to_unicode()
 */
static function unicode_to_utf8( &$str ) {
...
/* UTF-8 wordwrap */
/******************/
static function utf8_wordwrap($str,$len,$what) {
    $from=0;
...
* Modified for Elxis CMS by Ioannis Sannos
*/
static function utf8_to_ascii($str, $unknown='?') {
    global $mosConfig_absolute_path;

    $UTF8_TO_ASCII = array();
    $UTF8_TO_ASCII_DB = $mosConfig_absolute_path.'/includes/utf8db';
    ...
    while ( $i < $len ) {
        ...
        if (isset($UTF8_TO_ASCII[$bank]) && array_key_exists($newchar, $UTF8_TO_ASCII[$bank])) {
            $strx .= $UTF8_TO_ASCII[$bank][$newchar];
        } else {
            $strx .= $unknown;
        }
        $i += $increment;
 ...
 * characters in UTF-8. It's not perfect but should match most cases of special chars.
...
$UTF8_SPECIAL_CHARS = array(
	...
	0x0024, 0x0025, 0x0026, 0x0027, 0x0028, 0x0029, 0x002b, 0x002c, 0x002f, 0x003b,
	0x003c, 0x003d, 0x003e, 0x003f, 0x0040, 0x005b, 0x005c, 0x005d, 0x005e, 0x0060, 
	0x007b, 0x007c, 0x007d, 0x007e, 0x007f, 0x0080, 0x0081, 0x0082, 0x0083, 0x0084,
	0x0085, 0x0086, 0x0087, 0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e,
	0x008f, 0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097, 0x0098,
	0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f, 0x00a0, 0x00a1, 0x00a2,
	0x00a3, 0x00a4, 0x00a5, 0x00a6, 0x00a7, 0x00a8, 0x00a9, 0x00aa, 0x00ab, 0x00ac,
	0x00ad, 0x00ae, 0x00af, 0x00b0, 0x00b1, 0x00b2, 0x00b3, 0x00b4, 0x00b5, 0x00b6,
	0x00b7, 0x00b8, 0x00b9, 0x00ba, 0x00bb, 0x00bc, 0x00bd, 0x00be, 0x00bf, 0x00d7,
	0x00f7, 0x02c7, 0x02d8, 0x02d9, 0x02da, 0x02db, 0x02dc, 0x02dd, 0x0300, 0x0301,
	0x0303, 0x0309, 0x0323, 0x0384, 0x0385, 0x0387, 0x03b2, 0x03c6, 0x03d1, 0x03d2,
	0x03d5, 0x03d6, 0x05b0, 0x05b1, 0x05b2, 0x05b3, 0x05b4, 0x05b5, 0x05b6, 0x05b7,
	0x05b8, 0x05b9, 0x05bb, 0x05bc, 0x05bd, 0x05be, 0x05bf, 0x05c0, 0x05c1, 0x05c2,
	0x05c3, 0x05f3, 0x05f4, 0x060c, 0x061b, 0x061f, 0x0640, 0x064b, 0x064c, 0x064d,
