...
class gacl_api extends gacl {
	...
	function sort_groups($group_type='ARO') {
		...
		$query  = 'SELECT group_id, parent_id, name FROM '. $table .' ORDER BY parent_id, name ';
		...
		while ($row = $rs->FetchRow()) {
			...
			$sorted_groups[$parent_id][$group_id] = $name;
		}
	...
	function get_group_children($group_id, $group_type = 'ARO', $recurse = 'NO_RECURSE') {
		...
		$query  = 'SELECT g1.group_id FROM '. $table .' g1';
	...
	function get_group_data($group_id, $group_type = 'ARO') {
		...
		$query  = 'SELECT group_id, parent_id, name, lft, rgt FROM '. $table .' WHERE group_id='. $group_id;
		$this->db->setQuery( $query );
		$row = $this->db->loadRow();
	 ...
	 * @param string Group Type, either 'ARO' or 'AXO'
	 */
	function get_group_parent_id($group_id, $group_type='ARO') {

		$this->debug_text("get_group_parent_id(): ID: $group_id Group Type: $group_type");
		...
		if (empty($group_id) ) {
			$this->debug_text("get_group_parent_id(): ID ($group_id) is empty, this is required");
			return false;
		}

		$query = 'SELECT parent_id FROM '. $table .' WHERE group_id='. $group_id;
        $this->db->setQuery($query);
        $rs = $this->db->query();
	...
	/*======================================================================*\
		...
		Purpose:	Gets a groups parents IDs
	\*======================================================================*/
	function get_group_parents($group_id, $group_type = 'ARO', $recurse = 'NO_RECURSE') {
		...
		switch (strtoupper($recurse)) {
			case 'RECURSE':
                $query .= "SELECT g2.group_id FROM ". $table ." g1 " ;
				$query .= "LEFT JOIN ". $table ." g2 ON g1.lft > g2.lft AND g1.lft < g2.rgt WHERE g1.group_id=". $group_id;
				break;
			case 'RECURSE_INCL':
				// inclusive resurse			
			    $query .= "SELECT g2.group_id FROM ". $table ." g1 ";
				...
				break;
			default:
			    $query .= "SELECT g2.group_id FROM ". $table ." g2 ";
				$query .= "WHERE g2.parent_id='".$group_id."'";				
		}

		$query .= " ORDER BY g2.lft";		
	...
	function get_root_group_id($group_type='ARO') {
		...
		$query = 'SELECT group_id FROM '. $table .' WHERE parent_id=0';
		$this->db->setQuery( $query );
	...
	function add_group($name, $parent_id=0, $group_type='ARO') {
		...
		if ($parent_id == 0) {
			...
			$query = 'SELECT group_id, lft, rgt FROM '. $table .' WHERE group_id='. $parent_id;
			$this->db->setQuery( $query );
            $row = $this->db->loadRow($query);
		...
		$query = 'INSERT INTO '. $table .' (group_id,parent_id,name,lft,rgt) VALUES ('. $insert_id .','. $parent_id .','. $this->db->quote($name) .','. $parent_rgt .','. ($parent_rgt + 1) .')';
		$this->db->setQuery ( $query );
		$rs = $this->db->query();
	 ...
	 *
	 * @param int Group ID #
	 * @param string Group Value -> not used, ignore
	...
	function edit_group($group_id, $name=NULL, $parent_id=NULL, $group_type='ARO') {
		...
		$query  = 'UPDATE '. $table .' SET '. implode(',', $set) .' WHERE group_id='. $group_id;
		$this->db->setQuery( $query );
		$rs = $this->db->query();
	...
	function _rebuild_tree($table, $group_id, $left = 1) {
		...
		$query = 'SELECT group_id FROM '. $table .' WHERE parent_id='. $group_id;
		$this->db->setQuery( $query );
		$rs = $this->db->loadRowList();
		...
		$query  = 'UPDATE '. $table .' SET lft='. $left .', rgt='. $right .' WHERE group_id='. $group_id;
		$this->db->setQuery( $query );
		$rs = $this->db->query();
	...
	function get_object_groups($object_section_value, $object_value, $object_type=NULL) {
		...
		$dbora = in_array($this->db->_resource->databaseType, array('oci8', 'oci805', 'oci8po', 'oracle')) ? 1 : 0;
		if ($dbora) {
			$query = "SELECT g.group_id, o.".$group_type."_id, gm.group_id AS member FROM ".$group_table." g";
		} else {
			$query = "SELECT g.group_id, o.".$group_type."_id, (gm.group_id IS NOT NULL) AS member FROM ".$group_table." g";
		}
		$query .= "\n LEFT JOIN ".$table." gm ON gm.group_id=g.group_id"
		."\n LEFT JOIN ".$object_table." o ON o.".$group_type."_id = gm.".$group_type."_id"
		."\n WHERE 
		o.section_value='".$this->db->getEscaped($object_section_value)."' 
		AND o.value='".$this->db->getEscaped($object_value)."'";
		$this->db->setQuery($query);
		$rs = $this->db->loadResultArray();
	...
	function get_object_id($section_value, $value, $object_type=NULL) {
		...
		$row = $rs[0]; ///EDO YPARXEI LATHOS, EITE OBJECT EITE loadResultArray =============> TODO
	...
	function add_object($section_value, $name, $value=0, $order=0, $hidden=0, $object_type=NULL) {
		...
		if (eUTF::utf8_strlen($name) >= 255 OR strlen($value) >= 230 ) {
	...
	function del_object($object_id, $object_type=NULL, $erase=FALSE) {
		...
		// Get ids of acl referencing the Object (if any) - Commented by Ioannis Sannos (elxis)
	...
	function _getGroup( $type, $value ) {
		...
		$database->setQuery( "SELECT g.*"
			. "\nFROM #__core_acl_{$type}_groups g"
			. "\nINNER JOIN #__core_acl_groups_{$type}_map gm ON gm.group_id = g.group_id"
			. "\nINNER JOIN #__core_acl_{$type} ao ON ao.{$type}_id = gm.{$type}_id"
	...
	protected function _getBelow( $table, $fields, $groupby=null, $root_id=null, $root_name=null, $inclusive=true, $oracle=0) {
		$root = new stdClass();
		$root->lft = 0;
		$root->rgt = 0;
		...
		if ($root_id) {
			...
			$this->db->setQuery( "SELECT lft, rgt FROM ".$table." WHERE name='".$root_name."'" );
			$this->db->loadObject( $root );
		}
		...
		if ($oracle) {
			$this->db->setQuery("SELECT ".$fields
				."\n FROM ".$table." g1"
				.$where
				.($groupby ? "\n GROUP BY ".$groupby : '')
				."\n ORDER BY g1.lft");
			...
			$this->db->setQuery("SELECT ".$fields
				."\n FROM ".$table." g1"
				."\n INNER JOIN ".$table." g2 ON g1.lft BETWEEN g2.lft AND g2.rgt"
				.$where
				.($groupby ? "\n GROUP BY ".$groupby : '')
				."\n ORDER BY g1.lft");	
		}
	...
	public function get_group_children_tree( $root_id=null, $root_name=null, $inclusive=true ) {
		$oracle = in_array($this->db->_resource->databaseType, array('oci8', 'oci805', 'oci8po', 'oracle')) ? 1 : 0;
		if ($oracle) {
			$tree = $this->_getBelow( '#__core_acl_aro_groups', 'g1.group_id, g1.name, 
			(SELECT COUNT(g2.name) FROM #__core_acl_aro_groups g2 WHERE g1.lft BETWEEN g2.lft AND g2.rgt) AS qqlevel',
			'g1.name, g1.group_id, g1.lft', $root_id, $root_name, $inclusive, 1);
		} else {
			$tree = $this->_getBelow( '#__core_acl_aro_groups', 'g1.group_id, g1.name, COUNT(g2.name) AS level',
			'g1.name, g1.group_id, g1.lft', $root_id, $root_name, $inclusive, 0);
		}

		if ($oracle) { //just to be sure that qqlevel converted to level (qq: Elxis db compatibility prefix)
			for ($i=0; $i<count($tree); $i++) {
				$otree = $tree[$i];
				if (!isset($otree->level)) { $otree->level = $otree->qqlevel; }
			}
		}
    ...
    check if a group has previledges over an other
    $type: EDIT (GROUP)/RENAME/EDITPERMS/DELETE/EDITUSER/DELETEUSER
    */
	function allow_edit ($mygid='0', $checkgid='0', $type='EDIT' ) {
	   $allowed = false;

        //not allowed editing ROOT, or USERS groups
    	if (( $checkgid != '17' ) && ( $checkgid != '28' )) {
    	   /*
           not allowed edit/delete own group for all users
           allowed to rename own group if user is super administrator
           allowed to edit permissions to his own group if user is super administrator
           */
            $step = false;
            switch ( $type ) {
                case 'EDIT':
                case 'DELETE':
                case 'DELETEUSER':
                    if ($mygid != $checkgid ) { $step = true; }
                break;
                case 'RENAME':
                case 'EDITUSER':
                    if (($mygid != $checkgid ) || ( $mygid == '25' )) { $step = true; }
                break;
                case 'EDITPERMS':
                    if (($mygid != $checkgid ) || ( $mygid == '25' )) { $step = true; }
                break;
            ...
            if ( $step == true ) {
                $cpars = $this->get_group_parents( $checkgid, 'ARO', 'RECURSE' );
                $mpars = $this->get_group_parents( $mygid, 'ARO', 'RECURSE' );
                $spars = $this->get_group_parents( '25', 'ARO', 'RECURSE_INCL' );

                if ( @in_array( $mygid, $cpars )) { //not allowed editing children group
                    return false;
                } elseif ( @in_array( $checkgid, $mpars )) { //allowed editing parent group
                    return true;
                } elseif ( @in_array( $mygid, $spars )) { //allowed if you are in a admin group and you edit frontend group
                    return true;
        	    }
            }
        }
