...
class FeedCreator extends HtmlDescribable {
	...
	*/
	...
	function iTrunc($string, $length) {
		if (eUTF::utf8_strlen($string) <= $length) {
			return $string;
		}
		
		$pos = eUTF::utf8_strrpos($string, ".");
		if ($pos >= $length-4) {
			$string = eUTF::utf8_substr($string, 0, $length-4);
			$pos = eUTF::utf8_strrpos($string, ".");
		}
		if ($pos >= $length*0.4) {
			return eUTF::utf8_substr($string, 0, $pos+1)." ...";
		}
		...
		if ($pos >= $length*0.4) {
			return eUTF::utf8_substr($string, 0, $pos)." ...";
		}
		
		return eUTF::utf8_substr($string, 0, $length-4)." ...";
	...
	function saveFeed($filename="", $displayContents=true) {
		...
		if ($feedFile) {
			...
			echo "<p>Error creating feed file, please make folder cache/ writable (permissions 777)</p><br />\n";
		}
	}
