<?php 
	...
	//initialize Elxis
    define('_VALID_MOS', 1);
    $elxis_root = str_replace('/mambots/editors/tinymce/jscripts/tiny_mce/plugins/ibrowser/scripts', '', str_replace(DIRECTORY_SEPARATOR, '/', dirname(__FILE__)));
    $loggedBack = (isset($_REQUEST['loggedBack'])) ? (int)$_REQUEST['loggedBack'] : 0;
    require_once($elxis_root.'/mambots/editors/tinymce/jscripts/tiny_mce/tiny_auth.php');

	//-------------------------------------------------------------------------
	// include configuration settings
	include($elxis_root.'/mambots/editors/tinymce/jscripts/tiny_mce/plugins/ibrowser/config/config.inc.php');
	include($elxis_root.'/mambots/editors/tinymce/jscripts/tiny_mce/plugins/ibrowser/langs/lang.class.php');
	//-------------------------------------------------------------------------

	//remove upload, delete, rename, create capabilities for frontend
	...
	if (isset( $_REQUEST['lang'] )) {
        $editor_lang = $_REQUEST['lang'];
    } else if (isset( $_COOKIE['elxis_alang'] )) {
        $editor_lang = $_COOKIE['elxis_alang'];
    } else if (isset( $_COOKIE['elxis_lang'] )) {
        $editor_lang = $_COOKIE['elxis_lang'];
    } else {
        $editor_lang = $cfg['lang'];
    }
    ...
    if (!file_exists($elxis_root.'/mambots/editors/tinymce/jscripts/tiny_mce/plugins/ibrowser/langs/'.$tinylang.'.php')) { $tinylang = 'en'; }

	// language settings	
	//$l = (isset($_REQUEST['lang']) ? new PLUG_Lang($_REQUEST['lang']) : new PLUG_Lang($cfg['lang']));
	$l = new PLUG_Lang($tinylang);
	$l->setBlock('ibrowser');
	//-------------------------------------------------------------------------	
	// parameters
...
<meta http-equiv="Content-Type" content="text/html; charset=<?php echo $l->getCharset(); ?>" />
<!--[if IE]>
    <style type="text/css">#rfiles form {margin-top: -12px;}</style>
<![endif]-->
</head>
<body onload="init(); hideloadmessage();" dir="<?php echo $l->getDir(); ?>">
<?php include 'loadmsg.php'; ?>
<form id="rfiles" name="rfiles" method="post" action="<?php echo $mosConfig_live_site; ?>/mambots/editors/tinymce/jscripts/tiny_mce/plugins/ibrowser/scripts/rfiles.php" enctype="multipart/form-data" target="_self">
  <input type="hidden" name="lang" value="<?php echo $l->lang; ?>" />
  <input type="hidden" name="loggedBack" value="<?php echo $loggedBack; ?>" />
...
<?php
	...
	function getItems($path, $valid, $list) {			
		global $cfg, $mosConfig_live_site;
		global $l;		
		...
		if ($handle = @opendir($path)) {			
			...
			foreach ($files as $filename => $ext) {										
				...
				if ($list == true || $list == 1) {
					...
					$src     = $mosConfig_live_site.'/mambots/editors/tinymce/jscripts/tiny_mce/plugins/ibrowser/scripts/phpThumb/phpThumb.php?src=' . absPath(str_replace($cfg['root_dir'],'', $path)) . basename($filename) . '&w=48&h=48&far=1&bg=ffffff&f=jpg'; 				
			...
			if ($size > 0) {			
				...
				foreach($arr as $key => $thumb) {
					if (in_array($key, $chkT)) {										
						...
						$phpThumb->q = 90; 									// compression level for jpeg
