<?php 
	...
	// ================================================
	// This Version is an Advanced version of ibrowser for Elxis CMS
	// Created by Ioannis Sannos (datahell@elxis.org)
	// ================================================

	define('_VALID_MOS', 1);
    /*
    ibrowser secutiry, by Ioannis Sannos
    */
    $elxis_root = str_replace( '/mambots/editors/tinymce/jscripts/tiny_mce/plugins/ibrowser', '', str_replace( DIRECTORY_SEPARATOR, '/', dirname(__FILE__) ) );
    @require_once( $elxis_root.DIRECTORY_SEPARATOR.'configuration.php');
    ...
    if ( !file_exists(realpath(dirname(__FILE__).'/langs/'.$tinylang.'.php'))) { $tinylang = 'en'; }
...
// ============================================================
	function insertImage() {
		var formObj = document.forms[0];
		var root_dir = '<?php echo $cfg['base_url']; ?>'; //elxis
		var args = {};
			...
			if (x[i].className == 'btnDown') {
				if (x[i].id == 'mbtn_po') { // popup mode
					if(formObj.chkP.checked) {								
						...
						if (args.popSrc !=''){ args.popSrc = root_dir + args.popSrc; } //elxis
...
<form id="iBrowser" name="iBrowser" method="post" action="scripts/rfiles.php" enctype="multipart/form-data" target="inSelFrame">
  ...
  <input type="hidden" id="loggedBack" name="loggedBack" value="<?php echo $loggedBack; ?>" />
                ...
                <div class="colLeft">
                  <div style="float: left;">
                    ...
                    <div class="rowDiv">
                      ...
                      <div id="inSelDiv">
                        <iframe name="inSelFrame" id="inSelFrame" src="scripts/rfiles.php?clib=<?php echo $clib; ?>&loggedBack=<?php echo $loggedBack; ?>" style="width: 100%; height: 100%;" scrolling="no" marginheight="0" marginwidth="0" frameborder="0"></iframe>
                      </div>
                    </div>
                  </div>
                </div>
